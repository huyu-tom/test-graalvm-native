package com.huyu.testgraalvmlogmaven.mybatis;
...
public class MyBatisNativeConfiguration {
  ...
  static class MyBaitsRuntimeHintsRegistrar implements RuntimeHintsRegistrar {
    ...
    public void registerHints(RuntimeHints hints, ClassLoader classLoader) {
      Stream.of(MapperFactoryBean.class, RawLanguageDriver.class, XMLLanguageDriver.class,
              RuntimeSupport.class, ProxyFactory.class, Slf4jImpl.class, Log.class,
              JakartaCommonsLoggingImpl.class, Log4j2Impl.class, Jdk14LoggingImpl.class,
              StdOutImpl.class, NoLoggingImpl.class, SqlSessionFactory.class, PerpetualCache.class,
              FifoCache.class, LruCache.class, SoftCache.class, WeakCache.class,
              SqlSessionFactoryBean.class, ArrayList.class, HashMap.class, TreeSet.class, HashSet.class)
  ...
  static class MyBatisBeanFactoryInitializationAotProcessor implements
    ...
    private void registerMapperRelationships(Class<?> mapperInterfaceType, RuntimeHints hints) {
      ...
      for (Method method : methods) {
        if (method.getDeclaringClass() != Object.class) {
          ...
          registerSqlProviderTypes(method, hints, SelectProvider.class, SelectProvider::value,
              SelectProvider::type);
          registerSqlProviderTypes(method, hints, InsertProvider.class, InsertProvider::value,
              InsertProvider::type);
          registerSqlProviderTypes(method, hints, UpdateProvider.class, UpdateProvider::value,
              UpdateProvider::type);
          registerSqlProviderTypes(method, hints, DeleteProvider.class, DeleteProvider::value,
              DeleteProvider::type);
          Class<?> returnType = MyBatisMapperTypeUtils.resolveReturnClass(mapperInterfaceType,
              method);
    ...
    @SafeVarargs
    private <T extends Annotation> void registerSqlProviderTypes(Method method, RuntimeHints hints,
  ...
  static class MyBatisMapperTypeUtils {
    ...
    private static Class<?> typeToClass(Type src, Class<?> fallback) {
      ...
      if (src instanceof Class<?>) {
        ...
        int index = (parameterizedType.getRawType() instanceof Class && Map.class.isAssignableFrom(
  ...
  static class MyBatisMapperFactoryBeanPostProcessor implements MergedBeanDefinitionPostProcessor,
      BeanFactoryAware {
    ...
    private void resolveMapperFactoryBeanTypeIfNecessary(RootBeanDefinition beanDefinition) {
      ...
      if (beanDefinition.getResolvableType().hasUnresolvableGenerics()) {
        ...
        //将之前ClassPathMapperScanner处理bean的定义信息里面增加了错误的构造(参数是字符串，正确的应该是class),导致
        //在aot模式下,注入实例化错误
